...
gq_file <- snakemake@input[["gq_file"]]
...
compute_lfo <- function(stanfile, stanfile_gq, standata, L) {
  ...
  # compute exact p_LFO
  p_lfo <- exact_lpd_full - exact_elpd_1sap

  # compute exact LFO-CV RMSE
  rmse <- overall_rmse(
    true_pars = list(pred = standata$Y[(L + 1):N]),
    draws = pred_exact, zero_padding = FALSE
  )

  # compute exact LFO-CV energy score
  energy <- energy_score(
    true_pars = list(pred = standata$Y[(L + 1):N]),
    draws = pred_exact, zero_padding = FALSE
  )
...
results <- compute_lfo(
  ...
  stanfile_gq = gq_file,
  standata = standata,
  L = floor(standata$T / 2) + 1 # initial n_obs for LFO
)
